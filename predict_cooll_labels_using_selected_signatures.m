%
% Purpose:
%           Train model on a training set and predict COOLL labels from a test set
%           using selected terms from the log signature.
% Input     
%           
% Effects: 
%
...
% selected terms are found using signature_predictor_importance.m
selected_terms =  [5     6     8    10    12    13    14];
Xtrain = features(idx_train,selected_terms);
ytrain = labels(idx_train);
Xtest = features(idx_test,selected_terms);
...
[trainedClassifier, validationAccuracy] = trainEnsembleSignatureSelected(tblTrn);
